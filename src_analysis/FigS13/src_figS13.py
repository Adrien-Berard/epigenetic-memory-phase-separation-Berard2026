...
src_figS13.py
----------
Supplementary Figure S13: replicated polymer time-series panels from LAMMPS dumps.
"""
...
def plot_all(timesteps, ids_list, arrays,
    ...
    for tag, _ in rows:
        ...
        if tag != last_tag:
            ...
            ax.set_xticklabels(['0','2','4','6','8',rf'$10\times5$'])
    ...
    if types_df is not None and types_ts is not None:
        ...
        ax.plot(types_ts, types_df["Epe1"].values,
                color=EPE1_COLOR, linewidth=1.4, label="Epe1")

        # ax.set_title("Swi6M and Epe1 count", fontsize=10, pad=3)
        ax.set_ylabel("Swi6* and Epe1\nCount")
        y_ticks = [0,13,26]
        ax.set_yticks(y_ticks)
        ax.grid(alpha=0.25)
        # ax.legend(fontsize=8, loc="upper right")
