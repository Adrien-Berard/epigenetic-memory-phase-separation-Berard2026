...
src_figS4_k1.py
----------
Supplementary Figure S4 (k1): phase-diagram scan k1 vs Swi6; write PDF/CSV from simulation folders.
"""
import os
...
X_PARAM = "p1"  # k1 (directory key)    # name of the parameter that varies along x-axis
Y_PARAM = "swi6"  # name of the parameter that varies along y-axis


CONDENSED_DIR = DATA_ROOT / "SPombe_MatRegion_Model" / "P1-Swi6ParameterScan_18-03-2026"
DILUTE_DIR    = DATA_ROOT / "SPombe_MatRegion_Model" / "P1-Swi6ParameterScan_18-03-2026"
...
OUTPUT_DIR    = DATA_ROOT / "SPombe_MatRegion_Model" / "GMM_PhaseDiagram" / "p1vsswi6"
...
def _rgb_phase_page(pdf, fhA_grid, fhM_grid, x_vals, y_vals, noise,
    ...
    # x_ticks_log = np.linspace(x_log[0], x_log[-1], 3)
    # y_ticks_log = np.linspace(y_log[0], y_log[-1], 3)
...
def run(dilute_dir, condensed_dir, output_dir, noise_values,
    ...
    with open(csv_path, mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["noise", x_param, y_param, "tauA_both", "tauM_both"])
        for key in sorted(all_tauA_both.keys()):
            noise, x_val, y_val = key
            writer.writerow([noise, x_val, y_val,
                             all_tauA_both.get(key), all_tauM_both.get(key)])
    print(f"\nSaved CSV: {csv_path}")

    # ============================================================
    # PDF — one RGB phase diagram per noise value
    # ============================================================
    pdf_path = os.path.join(output_dir, "time_frac_phase_diagrams_p1_swi6.pdf")
    with PdfPages(pdf_path) as pdf:
        for noise in noise_values:
            noise_keys = [k for k in all_tauA_both if k[0] == noise]
            if not noise_keys:
                continue
    ...
    x_param,
    y_param,
    "time_frac_phase_diagrams_full_p1swi6_no_ylabels.svg",
)
