...
Panel order (top → bottom), all sharing the same x-axis:
  (a)  Figure3a.svg         (full-width, first row)
  (b)  Snapshots            M_state_polymer_full_box.png  |  A_state_polymer_full_box.png
  (c)  Kymograph            (dump, stacked)
  (d)  Polymer type counts  A/U/M  (one per polymer, from dump)
  (e)  Swi6M count          (types1.dat, green)
  (f)  Rg                   (r2.dat col c_R2[0], grey)
...
Usage:
    python figure3.py                        # uses default filenames
    python figure3.py my_dump.dat            # custom dump filename
    python figure3.py --out figure.png       # save instead of show
    python figure3.py --rg r2.dat --timeline replication_timeline.dat \\
                      --types types1.dat
"""
